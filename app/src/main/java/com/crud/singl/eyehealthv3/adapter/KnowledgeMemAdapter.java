...
import com.crud.singl.eyehealthv3.introHealth.UserDetailKnowledgeActivity;
 ...
 * */
public class KnowledgeMemAdapter extends BaseAdapter {

    Context mem_k;
    ArrayList<Knowledge> knowledges;

    public KnowledgeMemAdapter(Context mem_k, ArrayList<Knowledge> knowledges) {
        this.mem_k = mem_k;
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        if(view==null)
        {
            view= LayoutInflater.from(mem_k).inflate(R.layout.eye_knowledge_model,viewGroup,false);
        }
        ...
        Glide.with(mem_k).load(image).into(imageKnow);
    ...
    {
        Intent i=new Intent(mem_k,UserDetailKnowledgeActivity.class);
        i.putExtra("NAME_KEY",details[0]);
        i.putExtra("DETAIL_KEY",details[1]);
        i.putExtra("IMAGE_KEY",details[2]);

        mem_k.startActivity(i);
